...
- `lib/features/library/presentation/widgets/save_chord_bottom_sheet_widget.dart`
...
- `lib/features/library/presentation/widgets/save_chord_bottom_sheet_widget.dart` — 신규
  - `SaveChordBottomSheetWidget(chord, position)` — `ConsumerWidget`
...
- `lib/features/library/presentation/screens/library_screen.dart` — 수정
  - `_showBottomSheet`를 `showCBottomSheet` + `SaveChordBottomSheetWidget`으로 교체
  - `chord`, `position` 데이터를 바텀시트로 전달

**완료 조건:**

- [ ] 라이브러리 코드 카드 탭 시 저장 선택 바텀시트가 표시됨
- [ ] 코드 full name이 바텀시트 상단에 올바르게 표시됨
- [ ] 2개 버튼 UI가 렌더링됨 (동작은 다음 Phase)
...
- `lib/shared/template/c_toast.dart` — 신규
  ...
  - 앱 테마 색상 적용
- `lib/features/library/presentation/widgets/save_chord_bottom_sheet_widget.dart` — 수정
  - "새로운 Box 생성" 버튼 탭 시:
    1. `Navigator.pop(context)` (바텀시트 닫기)
    2. `showCDialog`로 새 Box 생성 폼 오픈
  - **새 Box 생성 폼 다이얼로그 내부 (`HookConsumerWidget` 또는 내부 StatefulWidget)**:
    ...
    - 만들기 탭 시:
      ...
      3. 성공: `CToast.show(context, '{제목}에 저장되었습니다.')`
      4. 실패: `CToast.show(context, '오류가 발생하였습니다.')`

**완료 조건:**

- [ ] "새로운 Box 생성" 탭 시 바텀시트 닫히고 CDialog 오픈됨
- [ ] 제목 입력 전 "만들기" 버튼 비활성 확인
- [ ] 새 Box 생성 후 `BoxDao.watchAll`로 목록에 즉시 반영됨
- [ ] 성공 / 실패 시 CToast 표시 확인
- [ ] `flutter analyze` 오류 없음
...
- `lib/features/library/presentation/widgets/select_box_bottom_sheet_widget.dart` — 신규
  - `SelectBoxBottomSheetWidget(chord, position)` — `HookConsumerWidget`
  - 상단 드래그 핸들 + "기존 Box에 추가" 헤더
...
- `lib/features/library/presentation/widgets/save_chord_bottom_sheet_widget.dart` — 수정
  - "기존 Box에 추가" 버튼 탭 시:
    1. `Navigator.pop(context)` (코드 저장 선택 바텀시트 닫기)
    2. `showCBottomSheet`로 `SelectBoxBottomSheetWidget` 오픈

**완료 조건:**

- [ ] "기존 Box에 추가" 탭 시 Box 목록 바텀시트 표시됨
- [ ] Box 목록이 `BoxListTileWidget`으로 올바르게 렌더링됨
- [ ] Box 없을 때 `BoxEmptyWidget` 표시 확인
- [ ] 중복 없는 항목 저장 시 토스트 확인
- [ ] 중복 항목 탭 시 확인 다이얼로그 표시, 각 응답 처리 확인
- [ ] `flutter analyze` 오류 없음
...
- [ ] `lib/features/library/presentation/screens/library_screen.dart` 기존 `_showBottomSheet` 파악 완료
- [ ] `lib/shared/data/db/dao/box_dao.dart`, `box_chord_dao.dart` DAO 메서드 파악 완료
- [ ] `lib/features/box/presentation/widgets/box_list_tile_widget.dart` 파라미터 파악 완료
