...
- "진동" — `CListTile.switchToggle()`, 현재 햅틱 상태 반영 / 즉시 저장
...
**기존 파일 (수정)**
- `lib/features/my/presentation/screens/my_screen.dart` — 섹션 구조 개편, 햅틱 토글·고객센터·정보 섹션 추가
...
- `lib/shared/constant.dart` — URL 상수 추가 (공지사항·문의하기·이용약관·개인정보처리방침)
...
- [x] MyScreen에 [설정] 섹션 헤더 + "화면 테마" + "진동" 타일 표시
- [x] 햅틱 토글이 현재 저장값 반영
- [x] 햅틱 토글 변경 시 shared_preferences에 즉시 저장
...
- [x] [정보] 섹션 헤더 표시
- [x] "앱 버전" 타일 trailing에 `v{버전명}` 텍스트 표시
- [x] 버전 로딩 중 `-` 또는 빈 문자열 표시
- [x] flutter analyze 오류 없음
...
- [x] CWebViewScreen이 title, url 파라미터를 받아 WebView 렌더링
- [x] 로딩 중 로딩 인디케이터 표시
- [x] 공지사항·문의하기·이용약관·개인정보처리방침 타일 탭 시 WebView 화면으로 이동
- [x] flutter analyze 오류 없음
...
- [x] "오픈소스 라이선스" 타일 탭 시 LicensePage로 이동
- [x] 패키지별 라이선스 목록 정상 표시
- [x] flutter analyze 오류 없음
